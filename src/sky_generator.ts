...
const create_star = (x_coord: number, y_coord: number, radius: number, override_attributes: Object = {}): XMLBuilderType => {
...
const galaxy_distribution = (width: number, height: number, num_stars: number, max_failed_iterations: number = 5000, radius_array = [10, 15, 20]): StarTuple[] => {
  ...
  while (star_idx < num_stars && failed_iterations < max_failed_iterations) {
      ...
      for (const star_props of distribution) {
          const [star_x, star_y, star_size] = [star_props[0], star_props[1], star_props[2]];
          // Check collisions
          if (Math.abs(x_coord - star_x) < (star_size + size) || Math.abs(y_coord - star_y) < (star_size + size)) {
              console.log(`Collision detected: star_x: ${star_x}, star_y: ${star_y}, star_size: ${star_size}, x_coord: ${x_coord}, y_coord: ${y_coord}, size: ${size}`);
              failed_iterations++;
              continue;
          }
      }
  ...
  if (failed_iterations == max_failed_iterations) {
      throw new Error("Max iterations reached, galaxy distribution might not be ideal");
  }
  return distribution;
};

const galaxy_generator = (width: number, height: number, num_stars: number): XMLBuilderType[] => {
    const stars: XMLBuilderType[] = [];
    const distribution = galaxy_distribution(width, height, num_stars);
    for (const star_props of distribution) {
        const [x_coord, y_coord, size] = [star_props[0], star_props[1], star_props[2]];
...
const sky_generator = (width: number, height: number, num_stars: number): string => {
    const stars = galaxy_generator(width, height, num_stars);
    const svg = create({ version: '1.0' })
        .ele('svg', { xmlns: 'http://www.w3.org/2000/svg', width: width, height: height });
